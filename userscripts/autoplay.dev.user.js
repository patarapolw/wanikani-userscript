    ...
    const obj = {
      ...
      load(s) {
        ...
        if (IMMERSION_KIT && Object.keys(IMMERSION_KIT).length) {
          ...
          const { decks = [], categories = [], tags = [] } = IMMERSION_KIT;
          /**
           *
           * @param {ImmersionKitExample[]} lookup
           */
          const sortLookup = (lookup) => {
            lookup = Array.from(lookup);
            [...categories, ''].map((cat) => {
              const inCat = cat
                ? lookup.filter((s) => s.category === cat)
                : lookup;

              [...decks, ''].map((deck) => {
                const inDeck = deck
                  ? inCat.filter((s) => s.deck_name === deck)
                  : inCat;

                [...tags, ''].map((tag) => {
                  const inTag = tag
                    ? inDeck.filter((s) => s.tags.includes(tag))
                    : inDeck;
                  lookup = inTag;
                });
              });
            ...
            return db.imKitSearch
              ...
              .then((r) => {
                ...
                const addExamples = (filterFn) => {
                  sentences.push(
                    ...sortLookup(
                      remainingExamples.filter((ex) => !filterFn(ex)),
                    ).map((ex) => formatImKit(ex)),
                  );
