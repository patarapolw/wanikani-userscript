(() => {
  ...
  var ExternalDefinition = class {
    ...
    async kanjipedia(q) {
      ...
      elHtml.querySelectorAll("script").forEach((el) => el.remove());
      const html = (((_a = elHtml.querySelector("#onkunList")) == null ? void 0 : _a.outerHTML) || "") + (((_b = elHtml.querySelector("#kanjiRightSection")) == null ? void 0 : _b.outerHTML) || "");
      elHtml.remove();
      return {
        html,
        url: trueUrl
    ...
    async weblio(q) {
      ...
      const elHtml = document.createElement("div");
      elHtml.innerHTML = await this.getFunction(url);
      const html = ((_a = Array.from(elHtml.querySelectorAll(".kiji")).filter((el) => (el.textContent || "").trim())[0]) == null ? void 0 : _a.outerHTML) || "";
      elHtml.remove();
      if (!html)
        return null;
      return {
